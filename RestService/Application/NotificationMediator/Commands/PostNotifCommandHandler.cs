...
namespace RestService.Application.NotificationMediator.Commands
{
    public class PostNotifCommandHandler : IRequestHandler<PostNotifCommand, CommandsDTO>
    {
        ...
        {
            var notifList = _context.notifs.ToList();
            ...
            {
                _context.notifs.Add(notifData);
            }
            await _context.SaveChangesAsync();

            
            var notif = _context.notifs.First(x => x.Title == request.Data.Attributes.Title);
            foreach(var k in request.Data.Attributes.Targets)
            {
                _context.notifLogs.Add(new NotificationLogs
                {
